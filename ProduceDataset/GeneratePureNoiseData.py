...
def whiten_like_yours(strain_ts: TimeSeries,
    ...
    # ostrain：原始 strain 只 crop，不做 highpass
...
def main():
    # ============ 你需要改的路径 ============
    PARAMS_CSV = "/mnt/3.6T/zkg/Development/VSCode/Projects/DoubleSeg/Util/更新版/params_for_generation_20W_jlx_2.csv"

    OUT_BASE = "/mnt/3.6T/zkg/Development/VSCode/Projects/DoubleSeg/Util/更新版/20W_noise_only"
    ...
    # 例如你要从 160W 开始命名：1_600_000
    # 这样 CSV idx=0 -> 文件名 signal_1600000.npy
    # =========================================================
    IDX_OFFSET = 1800000
    ...
    RESUME_MODE = "manual"
    RESUME_FROM_CSV_IDX = 148392    # manual 模式生效：例如 12345 表示从 12346 继续
    ...
    for _, row in df.iterrows():
        idx_val = int(row["idx"])  # CSV idx（不变）
        ...
        try:
            # 1) 仍然生成波形只为了得到长度 N、start_time、以及 PSD 的 delta_f
            ...
            # =====================================================
            # ✅ 核心最小改动：纯噪声 + 纯0信号
            # signal_ts 全 0；strain_raw = noise
            # snr 不需要调整 -> 这里固定 0
            # =====================================================
            target_snr = 0
            signal_ts = TimeSeries(np.zeros(N, dtype=np.float64), delta_t=delta_t)
            signal_ts.start_time = h_ifo.start_time
            ...
            print(f"Saved CSV idx={idx_val} -> file idx={file_idx} ifo={ifo} snr={target_snr} "
                  f"shift={shift} overlap={overlap} len={len(sig_concat)}")
